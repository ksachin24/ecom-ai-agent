...
const TABLE_SCHEMAS = {
    'product_ad_sales_metrics': `
        CREATE TABLE IF NOT EXISTS product_ad_sales_metrics (
            ...
            clicks INTEGER,
            units_sold INTEGER,
            PRIMARY KEY (date, item_id)  /* <-- CHANGED HERE */
        );
    `,
    ...
    'product_eligibility': `
        CREATE TABLE IF NOT EXISTS product_eligibility (
            ...
            message TEXT,
            PRIMARY KEY (eligibility_datetime_utc, item_id) /* <-- CHANGED HERE */
        );
    `
};
...
export default function App() {
                    ...
                    Papa.parse(csvContent, {
                        ...
                        complete: (results) => {
                            ...
                            try {
                                ...
                                // Using INSERT, if PRIMARY KEY (item_id) is duplicated, it will throw an error
                                // For this project, we assume item_id is unique per table, or errors on duplicates are acceptable
    ...
    const askQuestion = async () => {
        ...
        try {
            ...
            // Provide the LLM with the exact database schema
            ...
            const payloadSql = {
                ...
                generationConfig: {
                    // Instruct LLM to return JSON specifically for SQL query
            ...
            console.log("LLM SQL Response:", resultSql); // For debugging
            ...
            if (resultSql.candidates && resultSql.candidates.length > 0 &&
                ...
                try {
                    ...
                    setError(`LLM returned invalid JSON for SQL: ${jsonText}. Error: ${parseError.message}`);
                    setIsLoading(false);
                    return;
                }
            } else {
                setError('Failed to get SQL query from LLM. Please try again.');
                setIsLoading(false);
                return;
            }
            ...
            // Provide LLM with query results and original question, ask for human-readable answer and chart details
            ...
            const payloadResult = {
                ...
                generationConfig: {
                    // Instruct LLM to return JSON for final answer and visualization
                    responseMimeType: "application/json",
                    responseSchema: {
                        type: "OBJECT",
                        properties: {
